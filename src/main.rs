...
type IOPair = (Vec<f32>, Vec<f32>);
struct Network {
  output_layer: HiddenLayer
}

impl Network {
  fn new(dim: &[usize], activation_fn: fn(f32) -> f32, activation_fn_der: fn(f32) -> f32) -> Network {
  ...
  fn test(&mut self, test_set: &[IOPair]) {
  ...
  fn train_batch(&mut self, batch: &[IOPair], training_speed: f32, cost_der: fn((&f32, &f32)) -> f32) {
  ...
  fn train(&mut self, training_set: &mut [IOPair], test_set: &[IOPair], batch_size: usize, 
...
fn leaky_relu_der(x: f32) -> f32 {
  if x > 0.0 { 1.0 } else { 0.01 }
}

fn quadratic_cost_der((a, y): (&f32, &f32)) -> f32{
  2.0*(a - y)
}
fn crossentropy_cost_der((a, y): (&f32, &f32)) -> f32{
  -a/y + (1.0 - y)/(1.0 - a)
}
...
fn get_mnist(train_len: u32, test_len: u32) -> (Vec<IOPair>, Vec<IOPair>) { // terrible terrible code but it gets the job done
  let Mnist {
    trn_img,
    trn_lbl,
    tst_img,
